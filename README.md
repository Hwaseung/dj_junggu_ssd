...
- ***카드매출_가맹점 > 소분류_코드*는 수치형**인 것에 반해, *카드매출_가맹점 > 업종소분류*는 ***업종대분류*의 문자가 맨 앞에 붙어서 문자형**으로 처리된다.
  ...
  e.g. G47212 → 47212
  ```
- 위에서 정의된 딕셔너리를 *카드매출_가맹점 > 업종대분류, 업종중분류, 업종소분류* 에 맵핑했다.
- 맵핑한 후에 결측치는 없었다.
---
### 1-3 카드매출_유입고객
...
- 시군구_상권 : 30140
- 광역시_상권 : 30
=> 두 컬럼은 각각 하나의 값만 있고 이는 대전 중구, 대전광역시이다. 해당 컬럼은 큰 필요가 없기 때문에 삭제한다.
...
- *카드매출_가맹점*과 같은 방법으로 진행
  ...
  - 딕셔너리를 *카드매출_유입고객 > 업종대분류, 업종중분류, 업종소분류*에 맵핑
- 컬럼 추가 후에는 결측치가 없었다.
